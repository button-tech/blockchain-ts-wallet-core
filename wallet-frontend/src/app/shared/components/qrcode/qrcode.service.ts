...
export class QrCode {
  ...
  render(options: Options, elem: ElementRef): void {
    new QRCode(elem.nativeElement, options);
  }

  read(canvasElement: HTMLCanvasElement, image: CanvasImageSource): string | Error {
    ...
    return encodedData ? encodedData.data : new Error('failed to parse QR Code');
  }
